...
class YOLOCutiePipeline:
    def __init__(self, detection_model_path, segmentation_model_path, cutie_model_path=None, confidence_threshold=0.5, show_boxes=False, low_mem=False, logo_path=None):
        ...
        if logo_path is not None:
            ...
            if logo is None:
                ...
                if logo.shape[2] == 3:
                    # no alpha channel → add a fully opaque alpha
    ...
    def _overlay_logo(self, frame, binary_mask):
        """
        Overlay self.logo_image at the centroid of the binary_mask region.
        - frame: BGR uint8 image (H×W×3)
        - binary_mask: uint8 array (H×W), with 1 where mask present
        """
        ...
        # Resize logo to ~30% of bounding box width (preserve aspect ratio)
        logo = self.logo_image
        logo_h, logo_w = logo.shape[:2]
        scale = (box_w * 0.3) / logo_w
        new_w = int(logo_w * scale)
        new_h = int(logo_h * scale)
        if new_w < 1 or new_h < 1:
            return frame  # too small, skip overlay

        logo_resized = cv2.resize(logo, (new_w, new_h), interpolation=cv2.INTER_AREA)

        # Top-left corner to place the logo
        x0 = cx - new_w // 2
        y0 = cy - new_h // 2
        x1 = x0 + new_w
        y1 = y0 + new_h
        ...
        # Recompute region inside frame
        logo_resized = logo_resized[: (y1 - y0), : (x1 - x0)]
        overlay = logo_resized[..., :3]    # BGR of logo
        alpha_mask = logo_resized[..., 3:] / 255.0  # normalize alpha to [0..1]

        # Blend logo onto frame
        roi = frame[y0:y1, x0:x1].astype(np.float32)
        logo_f = overlay.astype(np.float32)
        alpha = alpha_mask.astype(np.float32)

        blended = alpha * logo_f + (1 - alpha) * roi
        frame[y0:y1, x0:x1] = blended.astype(np.uint8)
        return frame

    def _create_segmented_image_from_masks(self, original_frame, masks):
        """Create segmented image from a list of mask arrays, blur masked regions, and overlay logo."""
        result_image = original_frame.copy()

        if not masks:
            return result_image

        # Precompute a blurred version of the entire frame
        blurred_full = cv2.GaussianBlur(original_frame, (21, 21), 0)
        ...
        for mask in masks:
            ...
            # Create a binary mask (0 or 1) at full resolution
            ...
            if self.logo_image is not None:
                result_image = self._overlay_logo(result_image, binary)
